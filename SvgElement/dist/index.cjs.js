...
var glWidget = require('@gl-widget/gl-widget');
...
var _function = function createBezierBuilder(opt) {
  ...
  function recursive(x1, y1, x2, y2, x3, y3, x4, y4, points, distanceTolerance, level) {
    ...
    var pi = Math.PI; // Calculate all the mid-points of the line segments
    //----------------------
...
var EVENT_END = 1;
var EVENT_START = 2;
var monotone = monotoneTriangulate; //A partial convex hull fragment, made of two unimonotone polygons
...
var Buffer = require$$0.Buffer; //Legacy pool support
...
var partitionInteriorContainsInterval = partition('!(lo>=p0)&&!(p1>=hi)', ['p0', 'p1']);
var partitionStartEqual = partition('lo===p0', ['p0']);
var partitionStartLessThan$1 = partition('lo<p0', ['p0']);
var partitionEndLessThanEqual = partition('hi<=p0', ['p0']);
var partitionContainsPoint = partition('lo<=p0&&p0<=hi', ['p0']);
var partitionContainsPointProper = partition('lo<p0&&p0<=hi', ['p0']); //Frame size for iterative loop
...
var BOX_DSTACK = pool.mallocDouble(INIT_CAPACITY$1); //Initialize iterative loop queue
...
function boxIntersectIter(d, visit, initFull, xSize, xBoxes, xIndex, ySize, yBoxes, yIndex) {
  ...
  while (top > 0) {
    ...
    if (state & 2) {
      redEnd = partitionStartLessThan$1(d, axis, redStart, redEnd, red, redIndex, hi);
    ...
    if (red0 < redEnd) {
      ...
      //  Points > mid point
      //
      var blue0 = median(d, axis, blueStart, blueEnd, blue, blueIndex);
    ...
    if (d === 1 && full) {
      ...
      if (m > 0) {
        ...
        if (d === 1) {
          ...
          //General case:  d>1
          retval = intersect(d, visit, full, n, redList, redIds, m, blueList, blueIds);
        }
...
function segmentsIntersect(a0, a1, b0, b1) {
  var x0 = orient$1(a0, b0, b1);
  var y0 = orient$1(a1, b0, b1);

  if (x0 > 0 && y0 > 0 || x0 < 0 && y0 < 0) {
    return false;
  }

  var x1 = orient$1(b0, a0, a1);
  var y1 = orient$1(b1, a0, a1);
...
var isBn = isBN; //Test if x is a bignumber
...
var bnToNum = bn2num; //TODO: Make this better
...
var ctz_1 = ctzNumber; //Counts the number of trailing zeros
...
var toFloat = roundRat; // Round a rational to the closest float
...
function add(a, b) {
  return rationalize_1(a[0].mul(b[1]).add(b[0].mul(a[1])), a[1].mul(b[1]));
}
...
var unlerp = function range(min, max, value) {
...
var F__glWidgetPlugins_SvgElement_node_modules_objectAssign = shouldUseNative() ? Object.assign : function (target, source) {
  var from;
...
function bezierTo(points, scale, start, seg) {
  adaptiveBezierCurve(start, set(tmp1, seg[1], seg[2]), set(tmp2, seg[3], seg[4]), set(tmp3, seg[5], seg[6]), scale, points);
}
...
function cdt2d(points, edges, options) {
  ...
  var cells = monotone(points, edges); //If delaunay refinement needed, then improve quality by edge flipping
...
function boundRat(r) {
  var f = toFloat(r);
  return [nextafter_1(f, -Infinity), nextafter_1(f, Infinity)];
...
function boundEdges(points, edges) {
  ...
  for (var i = 0; i < edges.length; ++i) {
    ...
    var a = points[e[0]];
    var b = points[e[1]];
    bounds[i] = [nextafter_1(Math.min(a[0], b[0]), -Infinity), nextafter_1(Math.min(a[1], b[1]), -Infinity), nextafter_1(Math.max(a[0], b[0]), Infinity), nextafter_1(Math.max(a[1], b[1]), Infinity)];
  }
...
function boundPoints(points) {
  ...
  for (var i = 0; i < points.length; ++i) {
    var p = points[i];
    bounds[i] = [nextafter_1(p[0], -Infinity), nextafter_1(p[1], -Infinity), nextafter_1(p[0], Infinity), nextafter_1(p[1], Infinity)];
  }
...
function cutEdges(floatPoints, edges, crossings, junctions, useColor) {
  ...
  for (i = 0; i < crossings.length; ++i) {
    ...
    var ee = edges[e];
    var ef = edges[f];
    var x = ratSegIntersect(ratVec(floatPoints[ee[0]]), ratVec(floatPoints[ee[1]]), ratVec(floatPoints[ef[0]]), ratVec(floatPoints[ef[1]]));
...
function dedupPoints(floatPoints, ratPoints, floatBounds) {
  ...
  for (var i = 0; i < ratPoints.length; ++i) {
    ...
    var xb = boundRat(p[0]);
    var yb = boundRat(p[1]);
    bounds.push([nextafter_1(xb[0], -Infinity), nextafter_1(yb[0], -Infinity), nextafter_1(xb[1], Infinity), nextafter_1(yb[1], Infinity)]);
...
function snapRound(points, edges, useColor) {
  ...
  var labels = dedupPoints(points, ratPoints); // 5. dedupe edges
...
var normalizePathScale_1 = normalizePathScale;

function normalizePathScale(positions, bounds$1) {
  if (!Array.isArray(positions)) {
    throw new TypeError('must specify positions as first argument');
  }

  if (!Array.isArray(bounds$1)) {
    bounds$1 = bounds(positions);
  }
...
var F__glWidgetPlugins_SvgElement_node_modules_svgMesh3d = svgMesh3d;
...
function svgMesh3d(svgPath, opt) {
  ...
  var i; // parse string as a list of operations

  var svg = parseSvgPath(svgPath); // convert curves into discrete points

  var contours = svgPathContours(svg, opt.scale); // optionally simplify the path for faster triangulation and/or aesthetics

  if (opt.simplify > 0 && typeof opt.simplify === 'number') {
    for (i = 0; i < contours.length; i++) {
      contours[i] = simplifyPath(contours[i], opt.simplify);
    }
  ...
  var bounds$1 = bounds(positions); // optionally add random points for aesthetics

  var randomization = opt.randomization;

  if (typeof randomization === 'number' && randomization > 0) {
    addRandomPoints(positions, bounds$1, randomization);
  }
  ...
  var cells = cdt2d_1(positions, edges, opt); // rescale to [-1 ... 1]

  if (opt.normalize !== false) {
    normalizePathScale_1(positions, bounds$1);
...
function addRandomPoints(positions, bounds, count) {
  ...
  for (var i = 0; i < count; i++) {
    ...
    randomFloat(min[0], max[0]), randomFloat(min[1], max[1])]);
  }
}
...
function traverseNodes(child, paths, attributes = {}) {
    for (let i in child.childNodes) {
        ...
        if (c.tagName) {
            if (c.getAttribute('display') == 'none')
                continue;
            // let node: any = {
            // }
...
class SvgGeometry extends glWidget.Geometry {
    constructor(svg, options) {
        ...
        var mesh = F__glWidgetPlugins_SvgElement_node_modules_svgMesh3d(paths.join(), {
            delaunay: false,
            scale: 4
        });
        console.log(mesh);
...
class SvgElement extends glWidget.RenderableElement {
